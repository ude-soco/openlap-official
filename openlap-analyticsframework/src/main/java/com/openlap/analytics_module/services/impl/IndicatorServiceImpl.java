...
public class IndicatorServiceImpl implements IndicatorService {
		...
		if (indicatorType == IndicatorType.BASIC || indicatorType == IndicatorType.MULTI_LEVEL) {
			...
			indicatorFullDetailResponse.setAnalyticsTechniqueParams(
					gson.fromJson(
							...
							new TypeToken<List<OpenLAPDynamicParam>>() {
							}.getType()));
	...
	@Override
	public IndicatorsAnalyzedResponse getAnalyzedIndicators(IndicatorsToAnalyzeRequest indicatorList) {
		...
		for (IndicatorToAnalyze indicatorId : indicatorList.getIndicators()) {
			Indicator foundIndicator = indicatorUtilityService.fetchIndicatorMethod(indicatorId.getIndicatorId());
		...
		return new IndicatorsAnalyzedResponse(indicatorsAnalyzedList);
	}
