import { useContext } from "react";
import { Autocomplete, TextField, Typography } from "@mui/material";
import Grid from "@mui/material/Grid2";
import { BasicContext } from "../../../../basic-indicator";
import { fetchActionOnActivitiesList } from "../../utils/filters-api";
import { AuthContext } from "../../../../../../../../setup/auth-context-manager/auth-context-manager";

export default function ActivityTypeSelection({ activity }) {
  const { api } = useContext(AuthContext);
  const { dataset, filters, setFilters } = useContext(BasicContext);

  const handleSelectActivityType = (value) => {
    const loadActionOnActivityList = async () => {
      try {
        const actionOnActivityList = await fetchActionOnActivitiesList(
          api,
          dataset.selectedLRSList,
          [value.id]
        );
        return actionOnActivityList;
      } catch (error) {
        console.error(`Failed to load the action on activity list`, error);
      }
    };
    setFilters((p) => {
      const actionOnActivityList = loadActionOnActivityList();
      const updatedActivities = p.selectedActivities.map((a) =>
        a.id === activity.id
          ? {
              ...a,
              selectedActivityType: value,
              actionOnActivityList: actionOnActivityList,
              selectedActionOnActivityList: [],
              activityList: [],
              selectedActivityList: [],
            }
          : a
      );
      return {
        ...p,
        selectedActivities: updatedActivities,
      };
    });
  };

  return (
    <>
      <Typography gutterBottom>Select Activity type</Typography>
      <Autocomplete
        disableClearable
        disablePortal
        fullWidth
        options={filters.activityTypesList}
        getOptionLabel={(o) => o.name}
        value={activity.selectedActivityType || null}
        onChange={(event, value) => {
          if (value) handleSelectActivityType(value);
        }}
        renderInput={(params) => (
          <TextField {...params} placeholder="Activity types" />
        )}
        renderOption={(props, option) => {
          const { key, ...restProps } = props;
          return (
            <li {...restProps} key={key}>
              <Grid container sx={{ py: 0.5 }}>
                <Grid size={{ xs: 12 }}>
                  <Typography>{option.name}</Typography>
                </Grid>
                <Grid size={{ xs: 12 }}>
                  <Typography variant="body2">{option.id}</Typography>
                </Grid>
              </Grid>
            </li>
          );
        }}
      />
    </>
  );
}
